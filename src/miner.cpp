...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setRÜNESSchedule(RÜNESDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = RÜNESDGP.getBlockSize(nHeight);
    minGasPrice = RÜNESDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = RÜNESDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    RÜNESTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractRÜNESTX resultConverter;
    if(!convert.extractionRÜNESTransactions(resultConverter)){
    ...
    for(RÜNESTransaction RÜNESTransaction : RÜNESTransactions){
        txGas += RÜNESTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + RÜNESTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(RÜNESTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, RÜNESTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RÜNESMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("RÜNESMiner : ProcessNewBlock, block not accepted");
    }
