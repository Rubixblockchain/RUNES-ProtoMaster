...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RÜNES);
    ...
    if (!settings.contains("nAnonymizeRÜNESAmount"))
        settings.setValue("nAnonymizeRÜNESAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeRÜNESAmount = settings.value("nAnonymizeRÜNESAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRÜNESAmount"))
        SoftSetArg("-anonymizeRÜNESamount", settings.value("nAnonymizeRÜNESAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in RÜNES.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRÜNESAmount:
            return QVariant(nAnonymizeRÜNESAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeRÜNESAmount:
            nAnonymizeRÜNESAmount = value.toInt();
            settings.setValue("nAnonymizeRÜNESAmount", nAnonymizeRÜNESAmount);
            emit anonymizeRÜNESAmountChanged(nAnonymizeRÜNESAmount);
            break;
        case CoinControlFeatures:
