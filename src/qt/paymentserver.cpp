...
const int RUNES_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString RUNES_IPC_PREFIX("Runes:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(RUNES_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(RUNES_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(RUNES_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Runes: click-to-pay handler");
    else
