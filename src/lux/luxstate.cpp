...
#include <util.h>
#include <main.h>
#include "RÜNESstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

RÜNESState::RÜNESState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = RÜNESState::openDB(_path + "/RÜNESDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

RÜNESState::RÜNESState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute RÜNESState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, RÜNESTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createRÜNESAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            RÜNES::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> RÜNESState::vins() const // temp
{
...
void RÜNESState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* RÜNESState::vin(dev::Address const& _a) const
{
    return const_cast<RÜNESState*>(this)->vin(_a);
}

Vin* RÜNESState::vin(dev::Address const& _addr)
{
...
// void RÜNESState::commit(CommitBehaviour _commitBehaviour)
// {
...
void RÜNESState::kill(dev::Address _addr)
{
...
void RÜNESState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address RÜNESState::createRÜNESAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void RÜNESState::deleteAccounts(std::set<dev::Address>& addrs){
...
void RÜNESState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void RÜNESState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
