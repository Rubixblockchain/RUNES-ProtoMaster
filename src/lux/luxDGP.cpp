...
bool RÜNESDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule RÜNESDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t RÜNESDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t RÜNESDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t RÜNESDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t RÜNESDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool RÜNESDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void RÜNESDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void RÜNESDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void RÜNESDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void RÜNESDGP::createParamsInstance(){
...
dev::Address RÜNESDGP::getAddressForBlock(unsigned int blockHeight){
...
void RÜNESDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RÜNESDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RÜNESDGP::parseStorageOneUint64(uint64_t& value){
...
void RÜNESDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule RÜNESDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
